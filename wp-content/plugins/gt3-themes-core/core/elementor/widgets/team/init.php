...
if ( ! class_exists( 'ElementorModal\Widgets\GT3_Core_Elementor_Widget_Team' ) ) {
	class GT3_Core_Elementor_Widget_Team extends \ElementorModal\Widgets\GT3_Core_Widget_Base {
		...
		public function get_name() {
			return 'gt3-core-team';
		}

		public function get_title() {
			return esc_html__( 'Team', 'gt3_themes_core' );
		}

		public function get_icon() {
			return 'gt3-core-elementor-icon eicon-person';
		}
		...
		protected function construct() {
			...
			add_action('wp_ajax_gt3_themes_core_team_load_items', array( $this, 'ajax_handler' ));
			add_action('wp_ajax_nopriv_gt3_themes_core_team_load_items', array( $this, 'ajax_handler' ));
		}
		...
		public function render_team_item( $posts_per_line, $single_member = false, $grid_gap = '', $link_post = '', $custom_item_height = '', $settings = false ) {
			...
			if ( strlen( $wp_get_attachment_url ) ) {
				...
				if (!empty($settings['type']) && $settings['type'] == 'type5') {
					$gt3_featured_image_size = 204;
					$img_ratio = 1;
				}
				...
				if ( $custom_item_height == 'yes' && $settings['type'] !== 'type5') {
			...
			if ((!empty($settings['type']) && ($settings['type'] == 'type3' || $settings['type'] == 'type5')) && $settings['link_post'] ==  'yes') {
				$view_more = '<div class="team_link"><a href="'.get_permalink( get_the_ID() ).'">'. apply_filters('gt3/core/render/team/more_link_title', esc_html__( 'View More', 'gt3_themes_core' )) .'</a></div>';
			} else {
				$view_more = '';
			}
				...
				$compile .= '<li class="item-team-member' . $post_cats_str . ( empty( $featured_image ) ? ' item-team--no_image' : '' ) . '">';
					$compile .= '<div class="item_wrapper">';
						$compile .= '<div class="item">';
							...
							if (! empty( $icon_str ) && $settings['show_social'] == 'yes' && $settings['type'] !== 'type5') {
							...
							$compile .= '<div class="team-infobox">';
		                        ...
		                        if ( ! empty( $team_info ) && is_array( $team_info ) && $settings['show_fields_type5'] == 'yes' && $settings['type'] == 'type5' ) {
									$compile .= '<div class="gt3_single_team_info">';
									foreach ( $team_info as $team_info_item ) {
										$compile .= '<div class="gt3_single_team_info__item">';
										$compile .= ! empty( $team_info_item['name'] ) ? '<h4>' . esc_html( $team_info_item['name'] ) . '</h4>' : '';
										$compile .= ! empty( $team_info_item['address'] ) ? '<a href="' . esc_url( $team_info_item['address'] ) . '" target="_blank">' : '';
										$compile .= ! empty( $team_info_item['description'] ) ? '<span>' . $team_info_item['description'] . '</span>' : '';
										$compile .= ! empty( $team_info_item['address'] ) ? '</a>' : '';
										$compile .= '</div>';
									}
									$compile .= '</div>';
								}
		...
		public function ajax_handler(){
			...
			$settings = array(
				'posts_per_line' => $_POST['posts_per_line'],
				'show_social' => $_POST['show_social'],
			...
			if ($query->found_posts) {
				while($query->have_posts()) {
					$query->the_post();
					$respond .= $this->render_team_item($_POST['query']['posts_per_page'], false, $_POST['grid_gap'], $_POST['link_post'], $_POST['custom_item_height'],$settings);
				}
			}
